...
$(document).ready(function () {
  ...
  $(document).on("click", "#editStaff", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        document.getElementById("staff_state_edit").value =
          response.adminname.state;
        document.getElementById("staff_district_edit").value =
          response.adminname.district;
        document.getElementById("staff_profile_photo_show").src =
          APP_URL + "/admin_profile_photo/" + response.adminname.profile_photo;
      },
    });
  });
  ...
  // Edit staff
  $("#staff_admin_name1").blur(function () {
    // console.log("edit vales");
    $.ajax({
      //create an ajax request to display.php
      type: "GET",
      url: "/staff_website_return",
      ...
      success: function (response) {
        ...
        // $("#responsecontainer").html(response);
        //alert(response);
        let name = $("#staff_admin_name1").val();
        let id = $("#edit_id").val();
        const nameCleaned = name.replace(/\s/g, "") + id;
        let email = nameCleaned + response;
        $("#staff_email1").val(email);
      },
    });
  });
});
